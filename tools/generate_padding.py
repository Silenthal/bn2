...
def create_object_list_with_pad(
    ...
    for index in range(len(srcList)):
        ...
        if following_offset < next_offset:
            ...
            outList.append(
                {"name": pad_base, "obj": pad_obj, "off": pad_off, "len": pad_len}
            )
    return outList
...
def fill_layout_off_len(layDesc):
    ...
    for part in layDesc:
        ...
        if fullpath.exists():
            ...
            pGp = subprocess.Popen(
                ["grep", "\\.text"], stdin=pOd.stdout, stdout=subprocess.PIPE
            )
            pSd = subprocess.Popen(
                ["sed", "-E", "s/ +/_/g"], stdin=pGp.stdout, stdout=subprocess.PIPE
            )
            pCt = subprocess.Popen(
                ["cut", "-d", "_", "-f4"], stdin=pSd.stdout, stdout=subprocess.PIPE
            )
            size = int("0x" + pCt.communicate()[0].decode("utf-8"), 0)
            srcList.append(
                {"name": part["name"], "obj": nameObj, "off": part["off"], "len": size}
            )
...
def main():
    ...
    def assemble(outputName: str, outputOffset: int, outputLength: int) -> None:
        pEcho = subprocess.Popen(
            [
                "echo",
                f'.incbin "{fileBaseRom}", 0x{outputOffset:X}, 0x{outputLength:X}',
            ],
