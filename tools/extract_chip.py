...
def codes(inFile: BinaryIO) -> str:
    codeList = [common.chip_code(get_byte(inFile)) for _ in range(6)]
...
def flag(inFile: BinaryIO) -> str:
    ...
    for _ in range(8):
        if fb & x != 0:
            fl.append(flags[x])
        x <<= 1
    return "nochipflag" if x == 0 else "chipflag " + " | ".join(fl)

def chiptype(inFile: BinaryIO) -> str:
    flags = {
        1: "Defense",
        2: "Barrier",
        3: "Recover"
    ...
    with open(inPath, mode="rb") as inFile:
        with open(outPath, mode="w", encoding="utf-8") as outFile:
            ...
            for i in range(args.count):
                ...
                outFile.write(f"        mb {get_byte(inFile)}\n")
                outFile.write(f"        {flag(inFile)}\n")
                outFile.write(f"        damage {get_short(inFile)}\n")
                outFile.write(f"        {library(inFile)}\n")
                outFile.write(f"        {abc(inFile)}\n")
                outFile.write(f"        {chiptype(inFile)}\n")
                # Add filler byte in chiptype macro
                get_byte(inFile)
                _ = get_int(inFile)
